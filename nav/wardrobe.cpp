...
Wardrobe::Wardrobe(QWidget* ptr): m_size(0)
{
    ...
    connect(root->ui->labelHeader, &QLabel::linkActivated, this, &Wardrobe::wardrobeHandler);
}
...
void Wardrobe::trashBox()
{
    ClearLayout(root->m_actLayout);
    root->ui->stackedWidgetObjForm->setCurrentIndex(1);
    makeActBtn(warStr(13));
...
void Wardrobe::wardrobeHandler(QString act)
{
    ClearLayout(root->m_actLayout);
    if (act == warStr(3))
    {
        ...
        else
        {
            ...
            msgbox->setText(warStr(0));
            msgbox->setStandardButtons(QMessageBox::Ok);
            msgbox->setDefaultButton(QMessageBox::Ok);
            msgbox->setStyleSheet("background-color: #ffffff; color: #464646; font-size: 16px; font-family: 'Serif';");
            msgbox->exec();
        }
    }
    else if(act == warStr(1))
    {
        ...
        root->ui->labelObjDesc->setText(warStr(2));
        makeActBtn(warStr(5));
    }
    else if(act == warStr(5))
    {
        viewWardrobe();
    }
    else if(act == warStr(7) + warStr(9))
    {
        root->videoWidg->setQUrl("qrc:/img/clothing/0/0.webm",900,460);
        root->player()->redressPanties(nullptr);
        root->ui->labelVideoDesc->setText(warStr(4)+warStr(10)+warStr(12));
        makeActBtn(warStr(5));
        root->ui->stackedWidgetObjForm->setCurrentIndex(2);
    }
    else if(act == warStr(8) + warStr(9))
    {
        root->videoWidg->setQUrl("qrc:/img/clothing/0/6.webm",872,600);
        root->player()->redressPanties(m_panties);
        root->ui->labelVideoDesc->setText(warStr(4)+warStr(11)+warStr(12));
        makeActBtn(warStr(5));
        root->ui->stackedWidgetObjForm->setCurrentIndex(2);
    }
    else if(act == warStr(13))
    {
        viewWardrobe();
    }
    else if(act == warStr(14))
    {
        trashBox();
    }
}
...
void Wardrobe::makeActBtn(QString txt)
{
    ObjActionButton* btnx = new ObjActionButton(txt);
    btnx->setText(txt);
    connect(btnx, &ObjActionButton::sigAction, this, &Wardrobe::wardrobeHandler);
...
{
    QString str[15];
    str[0] = "<p style='color:red'>Вы стесняетесь в таком виде появиться в комнате.</p>";
    str[1] = "Снять одежду";
    str[2] = "Вы полностью разделись";
    str[3] = "Выйти";
    str[4] = "Вы ";
    str[5] = "Назад";
