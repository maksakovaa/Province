    ...
    case actBed2:
        {
            ...
            root->m_render->rendImagePage(root);
            root->m_render->getImagePtr()->setText(text);
            root->m_render->getImagePtr()->setStyleSheet("font-size: 160px");
            root->m_render->getTextPtr()->clear();
        }
        break;
    case actBed6:
    case actBed7:
...
void Bed::sleepInBed()
{
    ...
    {
        root->m_render->addText(actStr(actBed3));
        root->updVStatus(mood, -3);
    }
    if(root->getVStatus(oversleep) == 1)
    {
        root->m_render->addText(actStr(actBed4));
    }
    makeButtons();
    //AUTOSAVE
}
...
void Bed::bedDreams()
{
    ...
    if(i > 27)
    {
        root->m_render->setImage("data/qwest/father/bath_voyer_1.jpg");
    }
    else
    {
        root->m_render->setImage("data/img/dreams/" + intQStr(i) + ".jpg");
    }
